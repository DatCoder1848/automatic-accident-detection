...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # --- CHẠY YOLO TRACKING ---
            results = model.track(frame, classes=[2, 3, 5, 7], persist=True, imgsz=1024, agnostic_nms=False, tracker="custom_tracker.yaml", conf=0.2, iou=0.7, verbose=False)
...
# ++++++++++++++++++++++++++++++++++++++++++++ Cập nhật Begin ++++++++++++++++++++++++++++++++++++++++++++ #
                                ...
                                # Tín hiệu 1: Phanh gấp và dừng lại (Dành cho xe không bị mất ID)
                                kinematic_crash_A = (a_A <= -6.0) and (v_A < 5.0)
                                kinematic_crash_B = (a_B <= -6.0) and (v_B < 5.0)
                                has_kinematic_crash = kinematic_crash_A or kinematic_crash_B

                                # Tín hiệu 2: Bốc hơi kép (Cả 2 xe chạm nhau và cùng mất ID biến thành 1 khối)
                                is_mutual_ghosting = is_ghost_A and is_ghost_B

                                # Nếu khoảng cách đủ gần VÀ (có dấu hiệu phanh gấp HOẶC cả 2 bốc hơi)
                                if is_close_enough and (has_kinematic_crash or is_mutual_ghosting):
